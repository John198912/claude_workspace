...
| **a-3.2.4-iterate** | 迭代优化引擎 | 五轮迭代改稿；含改稿模式判别（AI稿放手/原稿少动）与少动纪律 | a-3.2.3 → s-3.3 |
...
| 编号 | 名称 | 职能 | 上游 → 下游 |
|------|------|------|------------|
| **s-4.2.2-adapt** | 多平台内容适配 | 各平台格式适配（含平台规则）；公众号段含排版决策方法论+反炫技阈值 | s-4.1 → 发布 |
...
| **s-3.4.2-ai-check** | AI写作检测 | AI味检测（V2.0 修正层：只审改动段+人写迹象白名单+聚集判定+不误伤；22特征降为信号库） | dbskill `/dbs-ai-check` | `skills/m3-creation/s-3.4.2-ai-check/README.md` |
...
**版本历史：**
- V2.3（2026-06-14）：原生整合 renwei 改稿少动纪律（a-3.2.4）、ai-check V2.0 方法论修正（s-3.4.2）、xiaohu 公众号排版决策方法论（s-4.2.2）、微信编辑器硬兼容规则（platform-rules）；质量门接入人味儿回归与排版反炫技
- V2.2（2026-06-12）：命令文档入仓、质量门 reviewer 子代理、配置模板入仓、四平台规则核验
